...
//订阅询价
MDAPI_API int WINAPI SubForQuoteRsp(char* instrumentsID[],int nCount)
{
	return pUserApi->SubscribeForQuoteRsp(instrumentsID,nCount);
}
///退订询价
MDAPI_API int WINAPI UnSubForQuoteRsp(char *ppInstrumentID[], int nCount)
{
...
//连接应答
MDAPI_API void WINAPI WINAPI RegOnFrontConnected(DefOnFrontConnected cb)
{
	_OnFrontConnected=cb;
}

//连接断开
MDAPI_API void WINAPI WINAPI RegOnFrontDisconnected(DefOnFrontDisconnected cb)
{
	_OnFrontDisconnected=cb;
}

//心跳
MDAPI_API void WINAPI WINAPI RegOnHeartBeatWarning(DefOnHeartBeatWarning cb)
{
	_OnHeartBeatWarning = cb;
}

//登录请求应答
MDAPI_API void WINAPI WINAPI RegRspUserLogin(DefOnRspUserLogin cb)
{
	_OnRspUserLogin=cb;
}

//登出请求应答
MDAPI_API void WINAPI WINAPI RegRspUserLogout(DefOnRspUserLogout cb)
{
	_OnRspUserLogout=cb;
}
//订阅行情应答
MDAPI_API void WINAPI WINAPI RegRspSubMarketData(DefOnRspSubMarketData cb)
{
	_OnRspSubMarketData=cb;
}

//退订行情应答
MDAPI_API void WINAPI WINAPI RegRspUnSubMarketData(DefOnRspUnSubMarketData cb)
{
	_OnRspUnSubMarketData=cb;
}

//深度行情通知
MDAPI_API void WINAPI WINAPI RegRtnDepthMarketData(DefOnRtnDepthMarketData cb)
{
	_OnRtnDepthMarketData=cb;
}

///订阅询价应答
MDAPI_API void WINAPI WINAPI RegRspSubForQuoteRsp(DefOnRspSubForQuoteRsp cb)
{
	_OnRspSubForQuoteRsp = cb;
}

///取消订阅询价应答
MDAPI_API void WINAPI WINAPI RegRspUnSubForQuoteRsp(DefOnRspUnSubForQuoteRsp cb)
{
	_OnRspUnSubForQuoteRsp = cb;
}

///询价通知
MDAPI_API void WINAPI WINAPI RegRtnForQuoteRsp(DefOnRtnForQuoteRsp cb)
{
	_OnRtnForQuoteRsp = cb;
}


MDAPI_API void WINAPI WINAPI RegRspError(DefOnRspError cb)
{
	_OnRspError= cb;
}
